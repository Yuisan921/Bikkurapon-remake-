 ...
 *
 *   当たり/はずれの決定自体はソフト側(app/lottery.py)の重み付き抽選で
 *   先に決まっているので、この機構に物理的なランダム性(実物のガチャガチャ
 *   のように山盛りのカプセルから偶然1個掴む、といった動き)は不要。
 *   「指定された1個を、残り1個になっても確実に落とす」信頼性だけを
 *   優先し、あえて「のど」をカプセルぎりぎりの太さにして単列を維持している。
 *
...
// throat_diameterは意図的にカプセルぎりぎりの太さにしてある。ここが緩いと
// 残り1個になった時にカプセルが「のど」の中で傾いて、ポケットが真下を
// 通過してもうまくハマらないことがある。当選/落選はソフト側(lottery.py)
// で決めているので、この機構に求められるのは「指定された1個を確実に
// 落とす」ことだけで、物理的なランダム性は不要。印刷後、実物のカプセルが
// 途中で引っかかる(渋すぎる)ようなら+3mm側に戻して調整すること
throat_diameter      = capsule_diameter + 2;   // カプセルが1列に並ぶ「のど」の径
